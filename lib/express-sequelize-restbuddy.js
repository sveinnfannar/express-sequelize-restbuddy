...
var restBuddy = module.exports = function (sequelize, options) {
  ...
  return function (req, res, next) {
    ...
    queryOptions.where = Sequelize.and(queryOptions.where, conditions);
...
var requestHandlers = Object.freeze({
  show: function show (model, queryOptions, options, req, res, next) {
    // TODO: Abstract findOne logic to remove duplications in update, head and destroy
    model.findOne(queryOptions)
      .then(function (item) {
        if (!item) {
          return res.status(404).json({ message: model.name + ' not found' });
        }
        return res.json(options.formatter(item));
      })
      .catch(next);
  },
  ...
  create: function create (model, queryOptions, options, req, res, next) {
    ...
    model.create(req.body)
      .then(function (item) {
        res.status(201).json(options.formatter(item));
      })
...
function parsePagination (req, options) {
  ...
  if (parseInt(req.query.perPage)) {
    pagination.limit = Math.min(parseInt(req.query.perPage), options.maxPerPage) || options.defaultPerPage;
    if (pagination.limit) {
...
restBuddy._getModelForResource = function getModelForResource (resource, sequelize) {
  ...
  var modelName = Utils.inflection.capitalize(Utils.singularize(resource.name));
