...
module.exports = function expressSequelizeRest (sequelize, options) {
  ...
  return function (req, res, next) {
    ...
    if (parseInt(req.query.perPage)) {
      queryOptions.limit = Math.min(parseInt(req.query.perPage), options.maxPerPage) || null;
    ...
    var conditions = applyConditionTransformers(
      filterUnknownConditions(parameters, model, _.keys(options.queryConditionTransformers)),
      options.queryConditionTransformers
    );
    ...
    model.findAll(queryOptions)
      .then(function (results) {
        if (requestType(req) === RequestTypes.Show) {
          if (results.length > 0) {
            res.json(options.format(results[0]));
          } else {
            res.status(404).json({ message: model.name + ' not found' });
          }
        } else {
          res.json(options.format(results));
        }
      })
      .error(next);
  }
...
function requestType (req) {
  ...
  if (req.method === 'GET' && _.last(routeParts)[0] === ':') { // Crude check to see if it's a param
    return RequestTypes.Show;
  } else if (req.method === 'GET') {
    return RequestTypes.List;
  }
  // TODO: Implement checks for other request types
}
 ...
 */
function filterUnknownConditions (conditions, model, queryTransformers) {
  var modelAttributes = _.keys(model.rawAttributes);
  var transformerKeys = _.keys(queryTransformers);
